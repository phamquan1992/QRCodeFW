...
export class LoginComponent implements OnInit {
    ...
    constructor(private dialog: MatDialog, public dialogRef: MatDialogRef<LoginComponent>, @Inject(MAT_DIALOG_DATA) public data: string,
        ...
        private messSrc: MessageService, private dataSrc: DataService, private localSrv: LocalStorageService, private datePipe: DatePipe) { }
        ...
        this.dataSrv.get('Authentications/Login?user=' + this.user_name + '&&pass=' + encodeURIComponent(this.pass_word)).subscribe(
            t => {
                ...
                if (nd.id == '0') {
                    ...
                    let time_now = this.datePipe.transform(new Date(), 'MM/dd/yyyy HH:mm:ss') as string;
                    this.localSrv.setStringValue('Login_Time', time_now);
    ...
    showDialog_changepass() {
        ...
        dialogConfig.width = this.innerWidth >= 1024 ? '560px' : "95%";
        dialogConfig.panelClass = "magrin_pane";
        this.dialog.open(ResetmailComponent, dialogConfig).afterClosed().subscribe(
            res => {
